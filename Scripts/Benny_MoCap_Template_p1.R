...
setwd("/Users/pcnnc/Documents/Opto_data/InProgress/03_+subj+/original")
source("03_+subj+_MoCapFunctions.R")
optoimport("03_+subj+")
...
optoLog=read.table("03_+subj+_optoReplant.txt",header=F,sep="")
x=length(optoLog$V1)
...
for (i in 1:x){
if(optoLog$V1[i]==10){
optoLog$translation[i] <- "LH_Block_Start"}
if(optoLog$V1[i]==11){
optoLog$translation[i] <- "RH_Block_Start"}
if(optoLog$V1[i]==12){
optoLog$translation[i] <- "Visible_Block_Start"}
if(optoLog$V1[i]==13){
optoLog$translation[i] <- "Hidden_Block_Start"}
if(optoLog$V1[i]==30){
optoLog$translation[i] <- "ERROR_lifted during loading/checking"}
if(optoLog$V1[i]==31){
optoLog$translation[i] <- "ERROR_no lift in 3s after stimulus"}
if(optoLog$V1[i]==32){
optoLog$translation[i] <- "ERROR_ lifted wrong hand during move"}
if(optoLog$V1[i]==33){
optoLog$translation[i] <- "ERROR_lifted wrong hand during stimulus presentation"}
if(optoLog$V1[i]==61){
optoLog$translation[i] <- "move epoch_1cm cube"}
if(optoLog$V1[i]==62){
optoLog$translation[i] <- "move epoch_2cm cube"}
if(optoLog$V1[i]==63){
optoLog$translation[i] <- "move epoch_3cm cube"}
if(optoLog$V1[i]==80){
optoLog$translation[i] <- "trial finished_okay"}
if(optoLog$V1[i]==90){
optoLog$translation[i] <- "experimenter_confirmed"}
if(optoLog$V1[i]==91){
optoLog$translation[i] <- "experimenter_denied"}
if(optoLog$V1[i]==92){
optoLog$translation[i] <- "experimenter_aborted"}
if(optoLog$V1[i]==93){
optoLog$translation[i] <- "auto_denied"}
if(optoLog$V1[i]==91){
optoLog$translation[i] <- "experimenter_denied"}
if(optoLog$V1[i]==100){
optoLog$translation[i] <- "end_of block"}
}
...
#QA
write.csv(a_9,file="b1_9.csv", row.names=F)
write.csv(b_9,file="b2_9.csv", row.names=F)
write.csv(c_9,file="b3_9.csv", row.names=F)
write.csv(d_9,file="b4_9.csv", row.names=F)
...
#QA
write.csv(a_81,file="b1_8.csv", row.names=F)
write.csv(b_81,file="b2_8.csv", row.names=F)
write.csv(c_81,file="b3_8.csv", row.names=F)
write.csv(d_81,file="b4_8.csv", row.names=F)
...
write.csv(df,file="03_+subj+_outputnumber.csv", row.names=F)
